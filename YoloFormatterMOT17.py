...
def create_yaml(root_dir, config, meta):
    ...
    yaml_content = \
f"#\n" \
f"# YOLO Dataset Configuration for {meta['dataset']} and auto-generated by\n" \
f"# YoloFormatterMOT17. Keep in mind that the path field can be changed using\n" \
f"# the following command in linux systems:\n" \
f"#\n" \
f"# ```bash\n" \
f"# sed -i 's/path: .*/path: \/absolute\/path\/to\/this\/dir/g' data.yaml\n" \
f"# ```\n" \
f"#\n" \
